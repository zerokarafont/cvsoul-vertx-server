...
import com.example.starter.service.app.ChatroomAppService
...
suspend fun chatroomApp(vertx: Vertx, schemaParser: SchemaParser, config: JsonObject): Router {
  ...
  router
    ...
    .handler(
      ...
      val message = requestEventbusPayload(
        action = ChatroomAPI.FETCH_CHATROOM_PAGINATION_LIST,
        params = params,
        body = body
      )

      val result = vertx.eventBus().request<JsonObject>(ChatroomAppService::class.java.name, message).await().body()
